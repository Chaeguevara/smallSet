...
def adversarial_validation(X, Y, n_splits = 10):
    ...
    for train_idx, test_idx in kfold.split(sparse_merge, y):
        ...
        log_reg = SGDClassifier(loss = 'lo_loss')
...
def print_model_metrics(y_test, y_test_prob,y_pred,lbl_to_idx, idx_to_lbl,label_list, confusion = False, verbose = False, return_metrics = False):
    ...
    pass_=False
    if pass_:
        label_=idx_to_lbl[idx] 
        precision, recall, threshold = precision_recall_curve(y_test, y_test_prob, pos_label = label_)
        ...
        y_test_pred = np.where(y_test_prob > best_threshold, label_, 0)
    
        # Calculate all metrics
        f1 = f1_score(y_test, y_test_pred, pos_label = label_, average = 'weighted')
...
def run_log_reg(train_features, test_features, y_train, y_test, lbl_to_idx, idx_to_lbl,label_list, alpha = 1e-4, confusion = False, return_f1 = False, verbose = False,):
    log_reg = SGDClassifier(loss = 'log_loss', alpha = alpha, n_jobs = -1, penalty = 'l2', random_state=1)
    log_reg.fit(train_features, y_train)
    y_test_prob = log_reg.predict_proba(test_features)
    y_pred = log_reg.predict(test_features)
    print(y_test_prob.shape)
    print_model_metrics(y_test, y_test_prob,y_pred,lbl_to_idx=lbl_to_idx,idx_to_lbl=idx_to_lbl,label_list=label_list, confusion = confusion, verbose = False, return_metrics = False)
    if verbose:
        print('F1: {:.3f} | Pr: {:.3f} | Re: {:.3f} | AUC: {:.3f} | Accuracy: {:.3f} \n'.format(*metrics))
    return log_reg